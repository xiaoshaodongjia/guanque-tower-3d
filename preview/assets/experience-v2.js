...
const ASSET_VERSIONS = {"guanque-fast.glb":"9f505e58ea21","guanque-exploration.glb":"c2b61f32c5bc","guanque-interior.glb":"f049a5bc2e2e","guanque-surroundings.glb":"03e9f89a11ac","audio/day.mp3":"49cd0b8c980b","audio/dusk.mp3":"6454c5fa922a","audio/part-bracket.mp3":"cb7869e37867","audio/part-column.mp3":"70b699446320","audio/part-podium.mp3":"1117c120b770","audio/part-ridge.mp3":"b3476ebc6869","audio/part-tile.mp3":"5b4bf3bd5fa1","audio/poem-0.mp3":"8fec4ef4aa5d","audio/poem-1.mp3":"d26aed6c7370","audio/poem-2.mp3":"68180cf8b284","audio/poem-3.mp3":"74b1c47f992a","audio/river.mp3":"9d19e6affa7e","audio/scene-0.mp3":"d6f137606103","audio/scene-1.mp3":"33e97dd0dad1","audio/scene-2.mp3":"47bd9556f9fc","audio/scene-3.mp3":"4d091c123093","audio/wind.mp3":"4aa2ff8d4ce0"};
  ...
  return { group, ensure, tick, get loaded() { return loaded; } };
    ...
    async function initialize(THREE, OrbitControls, GLTFLoader) {
      ...
      function parkOverviewCamera() {
        ...
        const fitY = Math.max(.001, tangent * (rect.bottom - rect.top) / rect.height * .9);
        let distance = 100;
        // Include both the low landscape and the tower without hiding either behind the UI.
        for (const bounds of [parkBounds, parkTowerBounds]) {
          for (const x of [bounds.min.x, bounds.max.x]) for (const y of [bounds.min.y, bounds.max.y]) for (const z of [bounds.min.z, bounds.max.z]) {
            const corner = V(x, y, z).sub(target), depth = corner.dot(direction);
            distance = Math.max(distance, depth + Math.abs(corner.dot(right)) / fitX, depth + Math.abs(corner.dot(up)) / fitY);
          }
        }
      ...
      renderer.setAnimationLoop(now => {
        ...
        const near = interiorExplorer.active ? .35 : clamp(camera.position.distanceTo(controls.target) / 35, .35, 75);
