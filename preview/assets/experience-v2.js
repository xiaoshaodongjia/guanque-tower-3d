...
const ASSET_VERSIONS = {"guanque-fast.glb":"9f505e58ea21","guanque-exploration.glb":"c2b61f32c5bc","guanque-interior.glb":"f049a5bc2e2e","guanque-surroundings.glb":"f6ddf3769a9a","audio/day.mp3":"49cd0b8c980b","audio/dusk.mp3":"6454c5fa922a","audio/part-bracket.mp3":"cb7869e37867","audio/part-column.mp3":"70b699446320","audio/part-podium.mp3":"1117c120b770","audio/part-ridge.mp3":"b3476ebc6869","audio/part-tile.mp3":"5b4bf3bd5fa1","audio/poem-0.mp3":"8fec4ef4aa5d","audio/poem-1.mp3":"d26aed6c7370","audio/poem-2.mp3":"68180cf8b284","audio/poem-3.mp3":"74b1c47f992a","audio/river.mp3":"9d19e6affa7e","audio/scene-0.mp3":"d6f137606103","audio/scene-1.mp3":"33e97dd0dad1","audio/scene-2.mp3":"47bd9556f9fc","audio/scene-3.mp3":"4d091c123093","audio/wind.mp3":"4aa2ff8d4ce0"};
...
function interiorWalkCanStand(level, x, z) {
  ...
  const columns = [];
  for (const cx of [-12,-8,8,12]) for (const cz of [-7,0,7]) columns.push([cx,cz]);
  for (const cx of [-4,0,4]) for (const cz of [-7,7]) columns.push([cx,cz]);
  columns.push([0,-2.6],[0,2.6]);
  if (level.index < 3) {
    for (const cx of [-24,24]) for (const cz of [-16,0,16]) columns.push([cx,cz]);
    columns.push([0,-16],[0,16]);
  }
  if (columns.some(([cx,cz]) => level.index < 3 ? nearBox(cx,cz,.55,.55) : Math.hypot(x-cx,z-cz) < diameter/2+radius)) return false;
...
function createInteriorWalk({ THREE, scene, camera, controls, canvas, sound, onExit, onFloor, setMode }) {
  ...
  function spawn(index) { return point([2,levels[index].z+eye,Math.min(8,levels[index].depth/2-.65)]); }
  function orient() {
  ...
  return {
    ...
    layout(){
      const width=window.innerWidth,height=window.innerHeight,sidebar=document.querySelector('.sidebar').getBoundingClientRect();
      camera.setViewOffset(width,height,width>720?-sidebar.right/2:0,width>720?0:Math.max(0,(height-sidebar.top)/2),width,height);
    ...
    tick(seconds){
      ...
      if(transition?.kind==='fade'){
        ...
        if(item.age>=.35&&!item.moved){item.moved=true;camera.position.copy(item.route?item.route[0]:spawn(item.to));yaw=0;pitch=-.04;orient();}
        if(item.age>=.7){
          if(item.route){
...
function createInteriorExplorer({ THREE, GLTFLoader, scene, renderer, camera, controls, sound, exterior, assetURL,
  ...
  $('interior-evidence').addEventListener('click', () => showDetail({
    ...
    paragraphs: [
      ...
      '目前没有完整建筑平面 CAD。楼板轮廓、轴线间距、交通区的绝对位置、墙厚、梯段及井道尺寸均为示意。未把缺失的房间用途和运营路线加入模型。',
  ...
  return { group, ensure, tick, get loaded() { return loaded; } };
    ...
    async function initialize(THREE, OrbitControls, GLTFLoader) {
      ...
      function selectParkStop(id) {
        ...
        flyTo(parkPoint(item.position), parkPoint(item.target),
